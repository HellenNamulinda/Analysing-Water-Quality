...
d3.csv("../data/water_quality_all.csv").then(data => {
...
const yScale = d3.scaleBand()
 .domain(data.map(d => d["Source type"].length === 0 ? "Others" : d["Source type"]))
 .range([0, height])
 .padding(0.1);
...
svg.selectAll(".bar-group")
 ...
 .attr("class", "bar")
 .attr("x", d => xScale(0))
 .attr("y", d => yScale(d.data["Source type"].length === 0 ? "Others" : d["Source type"]))
 .attr("width", 0)
 .attr("height", yScale.bandwidth())
 .transition()
 .duration(1000) .delay((d, i) => i * 100)
 .attr("x", d => xScale(d[0]))
